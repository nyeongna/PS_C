    ...
    while (1)
    {
        ...
        for (i=1; i<=N; i++) {
            for(j=1; j<=N; j++) {
                ...
                if (tree_list[i][j].size()==0) continue;
                ...
                for(k=0; k< tree_list[i][j].size(); k++) {
                    ...
                    if (ground[i][j] >= tree_list[i][j][k]) {
                        ...
                        tmp.push_back(tree_list[i][j][k]+1);
                    }
        ...
        for(i=1; i<=N; i++) {
            for(j=1; j<=N; j++) {
                ...
                if (dead_list[i][j].size()==0) continue;

                for(k=0; k<dead_list[i][j].size(); k++) {
